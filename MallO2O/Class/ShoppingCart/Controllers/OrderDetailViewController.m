...
@interface OrderDetailViewController ()<UIWebViewDelegate ,PayPalPaymentDelegate, PayPalFuturePaymentDelegate, PayPalProfileSharingDelegate>
...
-(void)clientAlipay{
    ...
    order.productName = @"O2O商城商品订单"; //商品标题
    order.productDescription = @"优质生活，尽在O2O商城！"; //商品描述
...
- (void)payPal{
    PayPalItem *item1 = [PayPalItem itemWithName:@"BT'S GOODS"
                                    withQuantity:1
                                       withPrice:[NSDecimalNumber decimalNumberWithString:payOrderArray[1]]
                                    withCurrency:@"RUB"
                                         withSku:payOrderArray[0]];
    NSArray *items = @[item1];
