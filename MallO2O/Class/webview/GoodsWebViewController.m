...
//    [_rightButton setBackgroundImage:[UIImage imageNamed:@"share_no"] forState:UIControlStateNormal];
...
- (void)share{
    ...
    _shareView.imageArray = @[@"UMS_wechat_session_icon@2x",@"UMS_wechat_timeline_icon"];
    _shareView.shareName = @[@"微信好友",@"朋友圈"];
...
- (void)clickBackGround:( UITapGestureRecognizer *)gesture{
    __weak typeof(self) vc = self;
    [UIView animateWithDuration:0.2 animations:^{
        vc.shareView.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_WIDTH/5);
        vc.blackView.alpha = 0;
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0) {
        ...
        [[UMSocialDataService defaultDataService]  postSNSWithTypes:@[UMShareToWechatSession] content:@"商城O2O" image:[UIImage imageNamed:@"share_icon"] location:nil urlResource:nil presentedController:self completion:^(UMSocialResponseEntity *response){
...
- (ShareView *)shareView{
    ...
    if (!_shareView) {
        ...
        [_shareView clickShareImg:^(NSInteger shareNumber) {
            if (shareNumber == 0) {
                [UMSocialData defaultData].extConfig.wechatSessionData.url = [vc.webViewUrl stringByAppendingString:@"&&a"];
                [UMSocialData defaultData].extConfig.wechatSessionData.title = vc.shopName;
                [[UMSocialDataService defaultDataService]  postSNSWithTypes:@[UMShareToWechatSession] content:@"我在商城o2o发现了一个不错的商品，赶快来看看吧。" image:imageView.image location:nil urlResource:nil presentedController:vc completion:^(UMSocialResponseEntity *response){
                    if (response.responseCode == UMSResponseCodeSuccess) {
                        [vc clickBackGround:nil];
                    }
                }];
            }
            if (shareNumber == 1) {
                [UMSocialData defaultData].extConfig.wechatTimelineData.url = [vc.webViewUrl stringByAppendingString:@"&&a"];
                [UMSocialData defaultData].extConfig.wechatTimelineData.title = vc.shopName;
                [[UMSocialDataService defaultDataService]  postSNSWithTypes:@[UMShareToWechatTimeline] content:@"我在商城o2o发现了一个不错的商品，赶快来看看吧。" image:imageView.image location:nil urlResource:nil presentedController:vc completion:^(UMSocialResponseEntity *response){
                    if (response.responseCode == UMSResponseCodeSuccess) {
                        [vc clickBackGround:nil];
                    }
                }];
