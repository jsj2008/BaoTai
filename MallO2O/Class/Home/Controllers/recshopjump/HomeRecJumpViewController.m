...
#pragma mark 获取网络数据
- (void)getIsCollect{
    NSString *url = [NSString stringWithFormat:@"http://o2omall.youzhiapp.com/action/ac_user/if_collect_shop"];
...
- (void)setCollectionStatusFromNet{
    ...
    if ([self.isCollect integerValue] == 0) {
        NSString *url = [NSString stringWithFormat:@"http://o2omall.youzhiapp.com/action/ac_user/collect_shop"];
        self.isCollect = @"1";
        NSDictionary *dic = @{
                              @"app_key" : url,
                              @"u_id" : [PersonInfoModel shareInstance].uID,
