...
- (void) initData {
    ...
    typeArray  = @[
                   ...
                   NSLocalizedString(@"editPerInfoemail", nil),
                   NSLocalizedString(@"editPerInfoPassport", nil)
                   ];
    _userName  = [[NSString alloc] init];
    _sexString = [[NSString alloc] init];
    _passport = [[NSString alloc] init];
}
...
- (void) settingUIAutoLayout {
    ...
    [_editTableView autoSetDimension:ALDimensionHeight toSize:200 * Balance_Heith];
...
//        _userName = infoDic[@"user_nickname"];
    ...
    if (_passport == nil || [_passport isEqualToString:@""]) {
        
        _passport = [PersonInfoModel shareInstance].passport;
    }
    ...
    [self swpPublicTooGetDataToServer:url parameters:dic isEncrypt:self.swpNetwork.swpNetworkEncrypt swpResultSuccess:^(id  _Nonnull resultObject) {
        ...
        if ([resultObject[@"code"] integerValue] == 200) {
            ...
            [PersonInfoModel shareInstance].passport = _passport;
            NSLog(@"%@",dic);
...
//            cell.sexString = [GetUserDefault(Person_Info) objectForKey:@"sex"];
        ...
        case 3:
            //            cell.sexString = [GetUserDefault(Person_Info) objectForKey:@"sex"];
            cell.email = [PersonInfoModel shareInstance].userName;
            break;
            
        case 4:
            //            cell.sexString = [GetUserDefault(Person_Info) objectForKey:@"sex"];
            cell.passport = [PersonInfoModel shareInstance].passport;;
            break;
