...
- (void) initData {
    ...
    _imgNameArray = @[@"my_onebuy",@"message_sel",@"mine",@"per_4",@"edit_password",@"per_5"];
    _typeTextArray = @[
                       NSLocalizedString(@"mineDestineRecord", nil),
                       NSLocalizedString(@"mineNews", nil),
//                       NSLocalizedString(@"mineOnline", nil),
    ...
    imgArray = @[@"1",@"2",@"3",@"4"];
    labelTextArray = @[
                       NSLocalizedString(@"myOrderPendingPayment", nil),
...
- (void)getCollectionNum{
    ...
    NSString *uId = @"0";
    //    if (GetUserDefault(U_ID) != nil) {
    //        uId = GetUserDefault(U_ID);
    //    }
    uId = [PersonInfoModel shareInstance].uID;
    ...
    if ([GetUserDefault(IsLogin) boolValue]) {
        [self swpPublicTooGetDataToServer:url parameters:jifenDic isEncrypt:self.swpNetwork.swpNetworkEncrypt swpResultSuccess:^(id  _Nonnull resultObject) {
            ...
            collectShopNum = resultObject[@"obj"][@"user_history_num"];
//            model.myMoney = resultObject[@"obj"][@"my_money"];
            model.myJifen = resultObject[@"obj"][@"integral"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *array = @[
                       ...
                       @{@"type":NSLocalizedString(@"mineHistoryGoods", nil),
...
//            [self.navigationController pushViewController:viewController animated:YES];
        ...
        if (index == 2) {
            SeeGoodsHistoryViewController *vc = [[SeeGoodsHistoryViewController alloc] init];
            [self.navigationController pushViewController:vc animated:YES];
        }
    }];
}
    ...
    [cell getIndex:^(NSInteger index) {
        ...
        switch (index) {
                ...
                NSLog(@"跳转余额充值");
            {
                ...
                viewController.webUrl = [NSString stringWithFormat:@"http://o2omall.youzhiapp.com/action/ac_shop/cz_card?u_id=%@",[PersonInfoModel shareInstance].uID];
                [self.navigationController pushViewController: viewController animated:YES];
            }
                break;
            case 21:
...
//                viewController.webUrl = [@"http://o2omall.youzhiapp.com/phone_web/yi_goods/yy_list" stringByAppendingFormat:@"?u_id=%@",[PersonInfoModel shareInstance].uID];
//                [self.navigationController pushViewController:viewController animated:YES];
                // 预订记录
                DestineRecordViewController *vc = [[DestineRecordViewController alloc] init];
                [self.navigationController pushViewController:vc animated:YES];
...
//            if (indexPath.row == 2) {
//                OnlingViewController *vc = [[OnlingViewController alloc] init];
//                [self.navigationController pushViewController:vc animated:YES];
//            }
            if (indexPath.row == 2) {
