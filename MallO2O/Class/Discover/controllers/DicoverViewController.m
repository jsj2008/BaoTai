...
- (void) initData {
    tableViewArray = @[
                       @[NSLocalizedString(@"dicoverBesom", nil), NSLocalizedString(@"dicoverSend", nil)],
...
//    }
    ...
    if (indexPath.section == 1 && indexPath.row == 1) {
        if ([self checkLogin] == YES) {
            [self pushWebView:NSLocalizedString(@"dicoverShakeNavigationTitle", nil) andWebUrl:[NSString stringWithFormat:@"http://o2omall.youzhiapp.com/phone_web/yy_yao/y_yao?u_id=%@",[PersonInfoModel shareInstance].uID]];
        }
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        if ([self checkLogin] == YES) {
            [self pushWebView:NSLocalizedString(@"dicoverOneMoneyNavigationTitle", nil) andWebUrl:[NSString stringWithFormat:@"http://o2omall.youzhiapp.com/phone_web/yi_goods/yy_goods?u_id=%@",[PersonInfoModel shareInstance].uID]];
        }
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        if ([self checkLogin] == YES) {
            [self pushWebView:NSLocalizedString(@"dicoverSendNavigationTitle", nil) andWebUrl:[NSString stringWithFormat:@"http://o2omall.youzhiapp.com/phone_web/yy_psy/yp_psy?u_id=%@",[PersonInfoModel shareInstance].uID]];
        }
    }
}
